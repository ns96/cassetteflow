...
static size_t
esp32_write(void *buf, size_t nframes, audio_element_handle_t self);
...
size_t
simpleaudio_tone(float tone_freq, size_t nsamples_dur, audio_element_handle_t self, unsigned int rate)
{
	...
	{ // SA_SAMPLE_FORMAT_S16:
		...
		for ( i=0; i<nsamples_dur; i++ )
	    	...
	    	if ( mag_s < 1 ) // "short epsilon"
	    		mag_s = 1;
	    	for ( i=0; i<nsamples_dur; i++ )
	    		short_buf[i*2+1] = short_buf[i*2] = lroundf( mag_s * sinf(SINE_PHASE_RADIANS) );  // left and right ch
	    }
	}
